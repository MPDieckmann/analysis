...
class CreateScreen {
  ...
  protected $options: CreateScreenOptions;
  protected $titleInput: HTMLInputElement;
  protected $textInput: HTMLTextAreaElement;
  protected $langSelect: HTMLSelectElement;

  constructor(options: CreateScreenOptions = {}) {
    // #region initialize components

    // initialize options
    this.$options = Object.assign<CreateScreenOptions, CreateScreenOptions>({
      title: "",
      text: ""
    }, options);
  ...
  private _getLine(entry: string) {
    var line: AnalysisLine = {
      orig: [],
      trans: ""
    };
  ...
  protected $create() {
    try {
      ...
      var titles: string[] = JSON.parse(localStorage.getItem("analysis-titles") || "[]");
...
var options: CreateScreenOptions = {};
try {
